...
console.log("sdr-scanner app.js loaded (layout-preserving + audio + PIN elevate) v2026-01-12c");
...
function updateControlLock() {
  ...
  // Disable control buttons (listen-only default)
  ...
  if (roleEl) {
    roleEl.textContent = locked ? "LISTEN ONLY" : "CONTROL ENABLED";
    roleEl.style.color = locked ? "#f59e0b" : "#10b981";
  }

  const toggle = document.getElementById("controlToggle");
  if (toggle) toggle.textContent = locked ? "Enable Control" : "Drop Control";
}
...
// ---------------------------
// Header audio controls (inject into existing header)
// ---------------------------
function ensureAudioControls() {
  ...
  // ---------------------------
  // ADDED: Control elevation UI (minimal)
  // ---------------------------
...
function updateSelectedButtons() {
  ...
  // ADDED: re-apply role lock after selection updates
  updateControlLock();
}
...
function handleMsg(msg) {
  ...
  // ---------------------------
  // ADDED: AuthInfo from server
  // ---------------------------
  if (msg.type === "AuthInfo") {
    const role = msg.data?.role || "viewer";
    setRole(role);
    if (msg.data?.error) alert(`Control denied: ${msg.data.error}`);
    return;
  }

  // ADDED: show forbidden cleanly
...
function connectWS() {
  ...
  ws.onopen = () => { setConn(true); log("ws connected"); setRole("viewer"); };
...
const TARGET_LATENCY_SEC = 0.10;  // aim for ~100ms behind realtime
const MAX_LATENCY_SEC    = 0.30;  // hard cap (~300ms). If exceeded, drop backlog.
const STARTUP_PRIME_SEC  = 0.06;  // initial prime (~60ms)
...
function handlePcmFrame(buf) {
  ...
  // If we ever fall behind (or first audio), prime close to realtime
  if (nextPlayTime < now) {
    nextPlayTime = now + STARTUP_PRIME_SEC;
  }

  // Current buffered latency (how far "in the future" we are scheduling)
  let latency = nextPlayTime - now;

  // If latency grows too large, DROP queued latency (snap back)
  if (latency > MAX_LATENCY_SEC) {
    nextPlayTime = now + TARGET_LATENCY_SEC;
    latency = nextPlayTime - now;
  }
